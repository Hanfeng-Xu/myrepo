...
#################  (1) 数据  ##############
#####数据
field <- read.csv(str_c(dir.path, "Data/clean_data/TRY_climtesp_anger.csv")) %>% dplyr::rename(Climate=Climate100)
head(field)

#野外权衡
field.trade <- read.csv(str_c(dir.path, '/Results/8.field_trade_offs100.csv'))
head(field.trade)

##植物园,每次更改气候变量
garden <- read.csv(str_c(dir.path, 'Data/clean_data/garden_data.csv'))  %>% na.omit() %>% dplyr::rename(Climate=Climate100)
head(garden)

##性状权衡
#植物园,每次更改气候变量
garden.trade <- read.csv(str_c(dir.path, '/Results/2.Garden_trade_offs100.csv'))
head(garden.trade)
...
#############筛选数据
###画线数据
dat <- subset(field.trade, select = c(group, n:Slope, Int, trait, comslope.t:comintint))
...
###保存
save_plot(str_c(dir.path, 'Results/9.garden_field100.pdf'), p.finall, base_height=10, base_width=15, dpi=300)
