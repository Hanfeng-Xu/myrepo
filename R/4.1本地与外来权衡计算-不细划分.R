...
##了解数据和函数
#### 数据路径
dir.path <- 'G:/Papers/Garden.final5/'
...
###计算性状对共同斜率所需的模型
model1 <- alist(sma(LNC~LCC*native,    data=dataset, log = 'xy'),
                sma(LDMC~LCC*native, data=dataset, log = 'xy'),
                sma(SLA~LCC*native,  data=dataset, log = 'xy'),
                sma(LDMC~LNC*native, data=dataset, log = 'xy'),
                sma(SLA~LNC*native,  data=dataset, log = 'xy'),
                sma(SLA~LDMC*native,   data=dataset, log = 'xy'))
###计算性状对共同截距所需的模型
model2 <- alist(sma(LNC~LCC+native,    data=dataset, log = 'xy'),
                sma(LDMC~LCC+native, data=dataset, log = 'xy'),
                sma(SLA~LCC+native,  data=dataset, log = 'xy'),
                sma(LDMC~LNC+native, data=dataset, log = 'xy'),
                sma(SLA~LNC+native,  data=dataset, log = 'xy'),
...
#保存
write.csv(sma.final, str_c(dir.path, '/Results/2.Native_trade_offs.csv'), row.names = F)
