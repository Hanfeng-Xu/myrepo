...
dataset <- field
#植物园数据集，不同气候阈值划分
#dataset <- garden %>% dplyr::select(Species:SLA, Climate=Climate100)
...
#保存
#write.csv(sma.final, str_c(dir.path, '/Results/2.Garden_trade_offs100.csv'), row.names = F)
write.csv(sma.final, str_c(dir.path, '/Results/2.Field_trade_offs.csv'), row.names = F)
